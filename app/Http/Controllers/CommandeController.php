...
{public function commande_ajout(Request $request)
{
    ...
    try {
        ...
        foreach ($paniers as $item) {
            $article = Article::lockForUpdate()->find($item->id_article);
            ...
            if ($article->stock < $quantite) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => "Stock insuffisant pour l'article {$article->nom_article}."
                ], 422);
            }
        ...
        if ($admin) {
            ...
            foreach ($articlesParBoutique as $btqId => $articlesBtq) {
                ...
                $this->sendFcmNotificationAdmin(
                    ...
                    $admin->device_token,
                    "Nouvelle commande 🎉",
                    "La boutique {$boutique->nom_btq} vient de recevoir une nouvelle commande."
                );
            }
        }
