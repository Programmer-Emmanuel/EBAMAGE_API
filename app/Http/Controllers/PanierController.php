    ...
    public function get_panier(Request $request)
{
    try {
        ...
        $normalize = fn($str) => preg_replace('/[^a-z]/', '', strtolower($str));
        ...
        $items = $paniers->map(function ($item) use ($normalize) {
            ...
            return [
                'hashid_panier_item' => Hashids::encode($item->id), // ID réel de la ligne panier
                'hashid' => Hashids::encode($article->id),
                'nom_article' => $article->nom_article ?? 'Nom indisponible',
                'quantite' => $item->quantite,
                'prix_unitaire' => $article->prix ?? 0,
                'image' => $image,
                'variations' => $variationsLabel,
                'prix_avec_quantite' => ($article->prix ?? 0) * $item->quantite,
            ];
        });

        $prix_total_panier = $items->sum('prix_avec_quantite');

        // Le dernier panier pour ID (ou premier si tu préfères)
        $lastPanier = $paniers->last();

        return response()->json([
            'success' => true,
            'cart' => $items,
            'id_panier' => $lastPanier ? Hashids::encode($lastPanier->id) : null, // <- ID correct
            'prix_total' => $prix_total_panier,
